...
| [0015](./0015-ambient-darkness-with-additive-light.md) | Dark layer in screen space with additive light pools | A slice in `Game.draw` is now the contract for every future danger/ambient source; a hostile puddle before the slice is invisible against the dark |
